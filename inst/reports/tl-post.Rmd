...
where reliance on a final clinical outcome (eg, overall survival) would
introduce unreasonable delays in care or, more rarely, when treatment
decisions that come after the surrogate outcome (eg, post-progression)
...
ratios for both outcomes we can extend the Daniel and Hughes for the NMA
...
predict the treatment effect on OS for the new therapy.
...
large effects on progression that shrink when considering OS.
...
Once the assumption of stability has been evaluated, the model can be
used to impute the missing treatment effect on outcome 2. For the sake
of illustration let's assume that our data follows the shared slope
assumption. Since we've simulated the data ourselves, we have the
ability to make one of the trials having a missing treatment effect on
OS, and then see how well our model predicts it. Since standard errors
are assumed known in meta-analysis we need to impute that as well. We'll
use a simple approach described in @achana2014network for illustrative
purposes. In **Figure 2** we can look at all of the treatment effects
our `r params$out2` alongside the known value for the missing
observation and it's predicted value an 95% credible interval. We can
subsequently use the mean or median predicted trial result our network
meta-analysis for `r params$out2` therefore giving us the opporyunity to
provide estimates of relative efficacy for `r params$focal` against
comparators even though we weren't able to measure it in the trial.

```{r fig-2, fig.width = 8, cache = TRUE}
